...
#define DATA_STORAGE_LEN 60000
static uint8_t						data[DATA_STORAGE_LEN];
...
void euscia0_isr(void)
{
	...
	if(status & EUSCI_A_UART_RECEIVE_INTERRUPT)
	{
		...
		data[dataIndex] = RXData;
		dataIndex = (dataIndex + 1) % DATA_STORAGE_LEN;
	}
...
void ConfigureUART(uint32_t Baudrate)
{
	...
	memset(data, 0xFF, DATA_STORAGE_LEN);
}
