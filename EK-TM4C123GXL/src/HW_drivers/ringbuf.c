...
#endif
/*
static short prior_state;
static inline short IntMasterDisable (void)
{
   prior_state = __get_interrupt_state ();
   __disable_interrupt ();
  return prior_state;
...
{
	__set_interrupt_state (prior_state);
...
void RingBufPeek(tRingBufObject *psRingBuf,uint8_t *data,uint8_t len)
{
    uint8_t i;
    
    for(i=0;i<len;i++)
    {
        data[i] = psRingBuf->pui8Buf[(psRingBuf->ui32ReadIndex+i)];
    }
}
